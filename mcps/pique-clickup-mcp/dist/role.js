...
const ROLE_TOOLS = {
    owner: new Set(ALL_TOOLS),
    editor: new Set(ALL_TOOLS),
    viewer: new Set([
        "get_task",
        "list_tasks",
        "list_tags",
...
export function parseRole(value) {
    ...
    if (v === "owner" || v === "editor" || v === "viewer")
        return v;
    throw new Error(`PIQUE_CLICKUP_ROLE invalido: "${value}". Use owner, editor ou viewer.`);
}
export function toolsFor(role) {
    return Array.from(ROLE_TOOLS[role]);
}
export function isAllowed(role, tool) {
