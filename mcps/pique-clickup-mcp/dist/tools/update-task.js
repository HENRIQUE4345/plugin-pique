    ...
    server.registerTool("update_task", {
        ...
        inputSchema: {
            ...
            custom_fields: z
                ...
                .optional()
                .describe("Custom fields pra atualizar, formato [{id, value}]. Cada campo vira POST separado em /task/{id}/field/{field_id}. Em caso de erro em algum, os demais ja aplicados ficam — retorna lista de sucesso e lista de falha."),
        },
        ...
        if (args.priority !== undefined)
            body.priority = PRIORITY_NUMBER[args.priority];
        ...
        let updated;
        if (hasMainBody) {
            updated = await ctx.client.putV2(`/task/${encodeURIComponent(args.task_id)}`, body);
        }
        ...
        if (hasCustomFields) {
            for (const field of args.custom_fields) {
                try {
                    await ctx.client.postV2(`/task/${encodeURIComponent(args.task_id)}/field/${encodeURIComponent(field.id)}`, { value: field.value });
        ...
        if (!updated) {
            updated = await ctx.client.getV2(`/task/${encodeURIComponent(args.task_id)}`);
        }
        const formatted = formatTask(updated);
