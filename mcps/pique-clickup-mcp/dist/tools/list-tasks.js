...
export function registerListTasks(server, ctx) {
    server.registerTool("list_tasks", {
        title: "List Tasks",
        description: "Filtra tasks por list_id, folder_id, space_id, assignees, statuses, e/ou intervalo de due_date. Pelo menos um escopo (list_id, folder_id, space_id) e obrigatorio se nao usar filtro de team. Retorna ate 100 tasks por chamada (paginadas pelo ClickUp). Datas formatadas em PT-BR.",
        inputSchema: {
        ...
        const buildQuery = () => {
            ...
            if (assigneeIds)
                assigneeIds.forEach((id) => q.append("assignees[]", String(id)));
            if (dueLt)
                q.set("due_date_lt", dueLt);
            if (dueGt)
                q.set("due_date_gt", dueGt);
            return q;
        };
        let path;
        if (list_id) {
        ...
        const res = await ctx.client.getV2(path);
