...
import { PRIORITY_NUMBER } from "../types.js";
import { validateCreateTask } from "../validations.js";
import { formatTask, ok, safeRun, taskToText } from "../tool.js";
export function registerCreateTaskFull(server, ctx) {
    server.registerTool("create_task_full", {
        title: "Create Task (Full)",
        description: "Cria uma task completa no ClickUp em UMA chamada (substitui create + update + dependency + attach do MCP oficial). Aplica todas as validacoes da Pique: verbo no infinitivo, descricao com 3 secoes obrigatorias, time estimate ≤4h, policies por pessoa (Gabriel/Daniel), gatilhos proibidos no Contexto. Em caso de falha apos task criada (ex: dependencia invalida), faz ROLLBACK deletando a task.",
        inputSchema: {
            list_id: z.string().describe("ID da list destino. Obrigatorio."),
            name: z
                .string()
                .min(3)
                .describe("Nome da task. DEVE comecar com verbo no infinitivo (Configurar, Criar, Revisar, Escrever, etc)."),
            markdown_description: z
                .string()
                .describe("Descricao em markdown. DEVE conter 3 secoes obrigatorias: '## Contexto', '## O que fazer', '## Criterio de pronto'. Cada secao com conteudo. Contexto NAO pode ter historico operacional do ClickUp (mesclagem, substituicao, IDs entre parenteses, etc)."),
            assignees: z
                .array(z.union([z.string(), z.number()]))
                .min(1)
                .describe("Assignees. Aceita IDs ClickUp (number) ou handles ('henrique', 'marco', 'gabriel', 'arthur', 'marcella', 'daniel')."),
            priority: z
            ...
            work_type: z
                ...
                .describe("Tipo de trabalho. pipeline = unidade de fluxo repetitivo. projeto = acao dentro de projeto com escopo unico. chamado = acao atomica isolada."),
            ...
            is_idea: z
                ...
                .optional()
                .default(false)
                .describe("Marca a task como ideia de conteudo (relaxa validacoes: dispensa secoes obrigatorias e teste das 4h)."),
            notify_all: z.boolean().optional().default(false).describe("Notificar assignees ao criar"),
        },
    }, async (args) => safeRun(async () => {
        ...
        const body = {
            ...
            assignees: assigneeIds,
            priority: PRIORITY_NUMBER[args.priority],
        ...
        // 5. Criar task
        const created = await ctx.client.postV2(`/list/${args.list_id}/task`, body);
        const taskId = created.id;
        const taskUrl = created.url ?? "";
        // 6. Se houver dependencias ou attaches, executar com rollback em caso de falha
        const dependencies_created = [];
        ...
        try {
            ...
            throw new Error(`Task ${taskId} foi criada mas falhou no passo seguinte. ROLLBACK aplicado (task deletada). Causa: ${detail}`);
        }
...
//# sourceMappingURL=create-task-full.js.map