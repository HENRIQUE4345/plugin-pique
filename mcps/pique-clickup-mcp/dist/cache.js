 ...
 */
export async function getHierarchy(client, forceRefresh = false) {
...
export function findList(hierarchy, listId) {
    for (const space of hierarchy.spaces) {
        ...
        if (list)
            return { space, folder: null, list };
    }
    return null;
}
/** Procura folder por id */
export function findFolder(hierarchy, folderId) {
    for (const space of hierarchy.spaces) {
...
async function fetchFromApi(client) {
    // 1. Spaces do workspace
    const spacesRes = await client.getV2(`/team/${WORKSPACE_ID}/space`, { archived: false });
    // 2. Para cada space, busca folders + folderless lists EM PARALELO
    const spaces = await Promise.all(spacesRes.spaces.map(async (s) => {
        const [foldersRes, looseListsRes] = await Promise.all([
            client.getV2(`/space/${s.id}/folder`, { archived: false }),
            client.getV2(`/space/${s.id}/list`, { archived: false }),
        ]);
        ...
        const looseLists = (looseListsRes.lists ?? []).map(parseList);
        return {
            id: s.id,
            name: s.name,
            folders,
